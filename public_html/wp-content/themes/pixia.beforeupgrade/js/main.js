...
function pixia_init() 
{
	...
	/*jQuery('.current-pirenko_portfolios-parent').each(function(index, element) {
		if (jQuery(this).parent().parent().children().attr('href')=="#")
		{
        jQuery(this).parent().parent().addClass('active');
		jQuery(this).parent().show();
		}
    });
	jQuery('#nav-main ul li ul li.active').each(function(index, element) {
		jQuery(this).parent().show();
    });*/
	//ADD FILTER CLASS TO PORTFOLIO SUBMENU BUTTONS
		...
		if (jQuery('#folio_classic').length)
		{
			...
			jQuery(window).smartresize(function()
			{
				if (jQuery('#folio_classic').length)
				{
					...
					jQuery('.portfolio_entry_li,.inset_shadow').css({'width':helper});
				}
			});
		}//if (jQuery('#folio_classic').length)
		if (jQuery('#folio_masonry').length)
	...
	function() 
	{
		...
		jQuery(this).find('.grid_single_title').css({'font-size':'40px'});
		...
		jQuery(this).find('.grid_single_title').css({'font-size':'60px','marginLeft':left_help});
