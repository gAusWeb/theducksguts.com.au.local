	...
	class pirenko_social_widget extends WP_Widget 
	{
		...
		function widget( $args, $instance ) 
		{
			...
			<div id="pirenko_social" class="cf" data-c_color="<?php echo $c_color; ?>">
                ...
                #pirenko_social a { width:41px; height:41px; display:block; outline:none; float:left; background-position:0 0; background-repeat: no-repeat; margin-right:6px; margin-bottom:6px; }
                #pirenko_social a:last-child { margin-right:0; }
                #pirenko_social a span { display:none; }
                #pirenko_social a:hover { background-position:100% 100%; }
                <?php
                $inside_counter=1;
                foreach ( $this->fields_array( $instance ) as $key => $data ) 
                {
                    ...
                    {
                        printf('#pirenko_social .%s { background-image:url(\'%s\'); }', substr($data['img'], 0, strpos($data['img'], '.')), esc_url(get_template_directory_uri().'/inc/theme_widgets/pirenko-social-links/icons/'.$instance['pir_icons'].'/'.$data['img'] ));
                        $inside_counter++;
                    }
                }
                ?>
				...
				foreach ( $this->fields_array( $instance ) as $key => $data ) 
				{
					if ( ! empty ( $instance[$key] ) ) 
					{
						printf( '<a class="%s" href="%s" %s><span pir_title="%s" data-pb-tint-opacity="1" data-c_color="'.$c_color.'" class="pir_icons %s">%s</span></a>', substr($data['img'], 0, strpos($data['img'], '.')), esc_url( $instance[$key] ), $new_window, esc_attr( $data['img_title'] ),$tips_class, esc_attr( $data['img_title'] ) );
						$inside_counter++;
					}
				}
				?>
